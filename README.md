...
This plugin for [Obsidian](https://obsidian.md) allows you to import YouTube video metadata into your Obsidian notes. It now also supports integration with TubeArchivist and SmartTranscripts for caption retrieval.

## Usage

Use the command "Create new YouTube note" to open the search window, where you can enter the YouTube URL, Video ID, or TubeArchivist URL. The plugin will then create a note from the configured template and replace variables in the template with video metadata from the YouTube API, TubeArchivist, or SmartTranscripts.
...
| Use SmartTranscripts         | Enable the use of SmartTranscripts for caption retrieval                                                                               |
| SmartTranscripts Base URL    | Base URL for the SmartTranscripts API (e.g., http://smarttranscripts-api.example.com)                                                  |
...
## SmartTranscripts Integration

This plugin now supports integration with SmartTranscripts for caption retrieval. To use this feature:

1. Set up your SmartTranscripts API and make note of its base URL.
2. In the plugin settings, enable the "Use SmartTranscripts" option and enter your SmartTranscripts Base URL.
3. When creating a new note, the plugin will use the SmartTranscripts API to retrieve captions if the option is enabled.

This integration allows you to enhance your video notes with accurate captions retrieved directly from the SmartTranscripts API.
